...
class Caffe_Model(object):
    ...
    def caffe_preprocess_and_compute(self, pimg, 
        ...
        if caffe_net is not None:
            ...
            H, W, _ = image.shape
            _, _, h, w = self.caffe_net.blobs['data'].data.shape
            h_off = max((H - h) / 2, 0)
            w_off = max((W - w) / 2, 0)
            crop = image[int(h_off):int(h_off + h), int(w_off):int(w_off + w), :]
            transformed_image = self.caffe_transformer.preprocess('data', crop)
            transformed_image.shape = (1,) + self.transformed_image.shape
    
            input_name = self.caffe_net.inputs[0]
            all_outputs = self.caffe_net.forward_all(blobs=output_layers,
